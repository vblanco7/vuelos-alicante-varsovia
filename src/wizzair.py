...
async def _intento(pw, origen: str, destino: str, hoy: str) -> tuple[list, list]:
    """Un intento completo: browser nuevo + warm-up + booking + 6 llamadas farechart."""
    ...
    try:
        ...
        print(f"Wizz Air: warm-up incompleto ({e}), continúo igualmente")
    ...
    if errores == 6:
        # ponytail: las 6 llamadas fallaron a la vez = la sesión Kasada no cuajó,
        # no que no haya vuelos. Señal de reintento al caller.
        raise RuntimeError("Wizz Air: las 6 llamadas farechart fallaron, sesión Kasada no establecida")

    return todos_outbound, todos_return
...
async def _buscar_async(origen: str, destino: str) -> tuple[list[dict], list[dict]]:
    ...
    async with async_playwright() as pw:
        for intento in range(2):
            try:
                todos_outbound, todos_return = await _intento(pw, origen, destino, hoy)
                break
            except RuntimeError as e:
                print(f"{e} — reintentando con sesión nueva" if intento == 0 else f"{e} — sin más reintentos")
